...
$$
A = \begin{matrix} 1 & 3 \\ 2 & 0 \\ 3 & 0 \end{matrix}.
$$

This is a rectangular matrix that has column vectors \\(v_1, v_2, \text{and} v_3\\) which define the column space (space defined by any linear combination of them). Defining the linear combination of them in this way we can understand the column space as a plane containing the region describable with them. The resulting graph looks as such!

{% include figure.liquid path="assets/img/column_space.png" title="Column space of two vectors creating a plane describe in grey."}

This is a unitary portion of the overall column space for simplicity, but it spans in all directions achievable by their linear combination. Feel free to play around with this yourself. Remember now that this is the described the column space because it is the vectors defined by the columns of matrix A.

Now, any point defined in the column space is easily projected with the inverse of matrix A back to where it would have lied on the 3D graph axes known so well! As seen in the image prior, I've defined a point in that space right at the centre of the column space of a half combination of both column vectors. The inverse of A is non-trivial but it is,

$$
A^{-1} = \begin{matrix} 5.61218077e-17 & 1.53846154e-01 & 2.30769231e-01 \\ 3.33333333e-01 & -5.12820513e-02 & -7.69230769e-02 \end{matrix},
$$
...
{% include figure.liquid path="assets/img/basis_column.png" title="Basis space and column space representation of two points, one line in the column space and one lying outside of it."}
...
Now for the mathematical intuition, consider a rectangular matrix A and it's SVD,
...
now is where we consider a goal to help us approach a definition, a sort of definition by characterisation. The goal is to find the pseudoinverse of course, by why we're here in the first place is to understand Least Squares! That is minimising x for,

$$
||Ax - b||,
$$

where A is our matrix that is not of equal size, b is somem point in basis space, and x is the value we're looking to minimise. We start by defining two terms,
...
Which drives each component to 0, for every \\(i > r\\). What this means is that we define every component up to the base of the column space after which nothing! In other words, this describes \\(\Sigma \in \textit{R}^{n \times m}\\), so x becomes,
